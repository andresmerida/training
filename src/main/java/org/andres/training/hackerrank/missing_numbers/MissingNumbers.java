...
public class MissingNumbers {
    ...
    private static List<Integer> missingNumbers(List<Integer> arr, List<Integer> brr) {
        ...
        Set<Integer> uniqueNumbers = new HashSet<>(brr);
        int frequenceA, frequenceB;

        for (Integer uniqueNumber : uniqueNumbers) {
            frequenceA = Collections.frequency(arr, uniqueNumber);
            frequenceB = Collections.frequency(brr, uniqueNumber);
            if (frequenceA != frequenceB) {
                missingNumbers.add(uniqueNumber);
            }
        }
